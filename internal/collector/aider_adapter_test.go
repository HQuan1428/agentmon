...
func TestAiderAdapterFailsClosedForSharedDefaultHistory(t *testing.T) {
	...
	rows, err := NewAiderAdapter(home).Discover(procscan.Snapshot{UID: 1000, Processes: []procscan.Process{
		{PID: 10, UID: 1000, StartTicks: 10, Comm: "aider", Args: []string{"aider", "--model=sonnet"}, Cwd: cwd},
		{PID: 11, UID: 1000, StartTicks: 11, Comm: "aider", Args: []string{"aider", "--model", "opus"}, Cwd: cwd},
	}})
...
func TestAiderAdapterFailsClosedWhenProcessesShareOpenHistory(t *testing.T) {
	...
	rows, err := NewAiderAdapter(home).Discover(procscan.Snapshot{UID: 1000, Processes: []procscan.Process{
		{PID: 10, UID: 1000, StartTicks: 10, Comm: "aider", Args: []string{"aider", "--model=sonnet"}, Cwd: cwd, Files: files},
		{PID: 11, UID: 1000, StartTicks: 11, Comm: "aider", Args: []string{"aider", "--model=opus"}, Cwd: cwd, Files: files},
	}})
	if err != nil || len(rows) != 2 {
		t.Fatalf("rows=%+v err=%v", rows, err)
	}
...
func TestAiderAdapterUsesLaunchModelEvidence(t *testing.T) {
	...
	rows, err := NewAiderAdapter(home).Discover(procscan.Snapshot{UID: 1000, Processes: []procscan.Process{
		{PID: 12, UID: 1000, StartTicks: 12, Comm: "aider", Cwd: configCwd},
		{PID: 13, UID: 1000, StartTicks: 13, Comm: "aider", Args: []string{"aider", "--model=argv-model"}, Env: map[string]string{"AIDER_MODEL": "env-model"}, Cwd: argCwd},
		{PID: 14, UID: 1000, StartTicks: 14, Comm: "aider", Env: map[string]string{"AIDER_MODEL": "env-model"}, Cwd: envCwd},
	}})
...
func TestAiderAdapterExpiresDoneRowAndRejectsPIDReuse(t *testing.T) {
	...
	snapshot := procscan.Snapshot{UID: 1000, Processes: []procscan.Process{{
		PID: 55, UID: 1000, StartTicks: 100, Comm: "aider", Cwd: cwd,
		Files: []procscan.OpenFile{{FD: 7, Path: input}, {FD: 8, Path: chat}},
	}}}
	...
	rows, err = adapter.Discover(procscan.Snapshot{UID: 1000, Processes: []procscan.Process{{
		PID: 55, UID: 1000, StartTicks: 200, Comm: "aider", Cwd: filepath.Join(t.TempDir(), "new"),
	}}})

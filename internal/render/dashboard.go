...
const (
	chromeLines = 8 // border(2) + header(3) + column header(2) + footer(1)
	minWidth    = 40
	defWidth    = 80
)
...
// right-aligned on that same middle line. The middle line's width is kept ≤ cw
// (falling back to a compact counter) so the frame never wraps it.
func header(cw int, c Counts, soundOn bool) string {
	...
	// measure, so the middle line lands at exactly cw and the frame won't wrap
	// it (manual gap math desyncs on double-width emoji like ⚡/🔊).
...
func Compose(width, height int, c Counts, soundOn bool, body []string, scroll int, empty bool) string {
	...
	inner := lipgloss.JoinVertical(lipgloss.Left, head, mid, foot)
